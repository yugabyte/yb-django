...
pip3 install -e .
git clone https://github.com/django/django.git --branch "stable/3.2.x"  $DJANGO_TESTS_DIR/django-3.2

# Install dependencies for Django tests.
sudo apt-get update
sudo apt-get install -y libffi-dev libjpeg-dev zlib1g-devel

cd $DJANGO_TESTS_DIR/django-3.2 && pip3 install -e . && pip3 install -r tests/requirements/py3.txt; cd ../../
...
cd $DJANGO_TESTS_DIR/django-3.2/tests
create_settings
